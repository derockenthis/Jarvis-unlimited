...
| ADK Runner | Google ADK runtime object that executes the agent and emits events. | `apps/backend/app/runtime/adk_runner.py` |
| Root Agent | The Jarvis ADK agent definition, model, instruction, and registered tools. | `apps/backend/app/agent/root_agent.py` |
| LiteLLM Model | ADK model adapter used to call OpenRouter models. | `apps/backend/app/agent/root_agent.py` |
| OpenRouter | Model provider configured through environment variables and LiteLLM. | `apps/backend/app/config.py` |
| ADK Session | Conversation/runtime state stored by ADK separately from durable memory. | `apps/backend/app/runtime/adk_runner.py` |
| `SqliteSessionService` | ADK SQLite session store used by the live chat runtime. | `apps/backend/app/runtime/adk_runner.py` |
...
| Agent Tool Wrapper | ADK-facing callable that closes over `PathPolicy` and exposes only model-facing parameters. | `apps/backend/app/tools/agent_tools.py` |
...
| Running MCP Tools | Concrete ADK tools resolved from MCP configs currently marked `running` and attached to the root agent for that chat request. | `apps/backend/app/mcp/adk_toolset.py`, `apps/backend/app/runtime/adk_runner.py` |
| Playwright Preset | Built-in MCP server preset for browser automation. | `apps/backend/app/mcp/presets.py` |
| Screen Sharing | User-controlled mode that lets the agent attach desktop screenshot and image-analysis tools to a chat request. | `apps/desktop/src/renderer/components/ChatPane.tsx`, `apps/backend/app/runtime/adk_runner.py` |
